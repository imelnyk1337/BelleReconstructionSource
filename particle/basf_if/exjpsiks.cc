...
test_module::event(BelleEvent *evptr, int *status)
{
  ...
  Mdst_vee2_Manager     &vee_mag     = Mdst_vee2_Manager::get_manager();
  ...
  for(Mdst_charged_Manager::iterator i = charged_mag.begin();
      i != charged_mag.end(); ++i){
  ...
  //fill vee lists with Mdst_Vee2 Data Base
  for(Mdst_vee2_Manager::iterator i = vee_mag.begin();
      i != vee_mag.end(); ++i){
  ...
  for(std::vector<Particle>::iterator i = electron.begin();
      i != electron.end(); ++i){
    for(std::vector<Particle>::iterator j = positron.begin();
	j != positron.end(); ++j){
  ...
  for(std::vector<Particle>::iterator i = muon_minus.begin();
      i != muon_minus.end(); ++i){
    for(std::vector<Particle>::iterator j = muon_plus.begin();
	j != muon_plus.end(); ++j){
      double mass = ((*i).momentum().p() + (*j).momentum().p()).mag();
      m_jpsi->accumulate(mass,1.);
      if(3.09693 - 0.05 <= mass && mass <= 3.09693 + 0.05){ // rough
  ...
  for(std::vector<Particle>::iterator i = kshort.begin();
      i != kshort.end(); ++i){
    for(std::vector<Particle>::iterator j = jpsi.begin();
	j != jpsi.end(); ++j){
      if(checkB0(*j,*i)){
	double mass = ((*i).momentum().p() + (*j).momentum().p()).mag();
...
//     for(std::vector<Particle>::iterator i = b0.begin();
// 	i != b0.end(); ++i){
