...
class d0kpi_mv_module : public Module
{
public:
  d0kpi_mv_module(void);
  ~d0kpi_mv_module(void){};
  void init(int*);
  void term(void){};
...
extern "C" Module_descr *mdcl_d0kpi_mv()
{
  d0kpi_mv_module *module = new d0kpi_mv_module;
  Module_descr *dscr = new Module_descr ( "d0kpi_mv", module );
  return dscr;
}

d0kpi_mv_module::d0kpi_mv_module(void)
  : m_ptypeD0("D0"),
    m_mc(1)
{
}
...
void
d0kpi_mv_module::begin_run(BelleEvent *dummy,
			  int *dummyStatus)
{
  //eid::init_data();
  return;
...
void
d0kpi_mv_module::init(int *dummyStatus)
{
  //dummy
  Ptype dummy("E-");
}
...
void
d0kpi_mv_module::event(BelleEvent *evptr, int *status)
{
  *status = -1;
  ...
  if(m_mc){
    ...
    if(evtMgr.count() != 0){
      if(evtMgr[0].ExpMC() != 2){
        m_mc = 0; // not MC
        dout(Debugout::INFO,"d0kpi_mv") << "(module) This analysis is in REAL DATA not MC DATA." << std::endl;
      }
    }
  }
...
void
d0kpi_mv_module::massD0Event(vector<Particle> &plist)
{
...
void
d0kpi_mv_module::writeHisto(vector<Particle> &plist,
			   BelleTuple *hist)
{
  for(int i=0;i<plist.size();++i){
    // lund(charge) info.
