...
class dsd0pi_module : public Module
{
public:
  dsd0pi_module(void);
  ~dsd0pi_module(void){};
...
private:
  ...
  void fitDstarEvent(vector<Particle>&,
		     const HepPoint3D &ipPosition,
		     const HepSymMatrix &ipError);
...
extern "C" Module_descr *mdcl_dsd0pi()
{
  dsd0pi_module *module = new dsd0pi_module;
  Module_descr *dscr = new Module_descr ( "dsd0pi", module );
  IpProfile::define_global(dscr);
  return dscr;
}

dsd0pi_module::dsd0pi_module(void)
  : m_ptypeD0("D0"),
    m_ptypeDS("D*+"),
    m_mc(1)
{
...
void
dsd0pi_module::begin_run(BelleEvent *dummy,
			  int *dummyStatus)
{
  //eid::init_data();
...
void
dsd0pi_module::init(int *dummyStatus)
{
  //dummy
  Ptype dummy("E-");
}
...
void
dsd0pi_module::event(BelleEvent *evptr, int *status)
{
  *status = -1;
  ...
  if(m_mc){
    ...
    if(evtMgr.count() != 0){
      if(evtMgr[0].ExpMC() != 2){
        m_mc = 0; // not MC
        dout(Debugout::INFO,"dsd0pi") << "(module) This analysis is in REAL DATA not MC DATA." << std::endl;
      }
    }
  }
  ...
  // Make D*+ (with No Cut)
  vector<Particle> DS;
  combination(DS, m_ptypeDS, D0, m_pionP);
  setUserInfo(DS);
  if(m_mc){
    setGenHepInfoR(DS);
  }
  ...
  {
    fitDstarEvent(DS,
		  IpProfile::position(1),
		  IpProfile::position_err(1));
    writeHisto(DS, m_hist);
  }
#else
  if(IpProfile::usable()){
    fitDstarEvent(DS,
...
void
dsd0pi_module::fitD0Event(vector<Particle> &plist)
{
...
void
dsd0pi_module::fitDstarEvent(vector<Particle> &plist,
			     const HepPoint3D &ipPosition,
			     const HepSymMatrix &ipError)
{
  for(int i=0;i<plist.size();++i){
    // production point (IP profile and D0 vector)
    kvertexfitter kf;
    kf.magneticField(BF);
    addTrack2fit(kf, plist[i].child(0)); // D0
    kf.initialVertex(ipPosition);
    kf.vertexProfile(ipError);
    unsigned err = kf.fit();
    if(err != 0)continue;
    ...
    addTrack2fit(kvf, plist[i].child(1)); // slow PI+
    kvf.initialVertex(kf.vertex());
    kvf.knownVertex();
    err = kvf.fit();
    if(err != 0)continue;

    double dm0 = plist[i].mass()-plist[i].child(0).mass();
    double dm1 = (kf.momentum(0)+kvf.momentum(0)).mag()-kf.momentum(0).mag();

    // mass fit with a point
    kmassfitter kmf;
    kmf.magneticField(BF);
    /*
    dout(Debugout::INFO,"dsd0pi") << kf.momentum(0) << std::endl;
    dout(Debugout::INFO,"dsd0pi") << kf.position(0) << std::endl;
    dout(Debugout::INFO,"dsd0pi") << kf.error(0) << std::endl;
    dout(Debugout::INFO,"dsd0pi") << kf.track(0).charge() << std::endl;
    dout(Debugout::INFO,"dsd0pi") << kf.track(0).mass() << std::endl;
    dout(Debugout::INFO,"dsd0pi") << kvf.momentum(0) << std::endl;
    dout(Debugout::INFO,"dsd0pi") << kvf.position(0) << std::endl;
    dout(Debugout::INFO,"dsd0pi") << kvf.error(0) << std::endl;
    dout(Debugout::INFO,"dsd0pi") << kvf.track(0).charge() << std::endl;
    dout(Debugout::INFO,"dsd0pi") << kvf.track(0).mass() << std::endl;
    */
    //dout(Debugout::INFO,"dsd0pi") << kf.track(0).momentum(0).mag() << ", " << kf.track(0).momentum(1).mag()
    // << ", " << kf.track(0).mass() << ", " << kf.momentum(0).mag() << std::endl;
    kmf.addTrack(kf.momentum(0),
		 kf.position(0),
		 kf.error(0),
		 kf.track(0).charge(),
		 kf.track(0).mass()); // D0
    kmf.addTrack(kvf.momentum(0),
		 kvf.position(0),
		 kvf.error(0),
		 kvf.track(0).charge(),
		 kvf.track(0).mass()); // slow PI+
    kmf.invariantMass(m_ptypeDS.mass());
    kmf.vertex(kf.vertex());
    //kmf.errVertex(kf.errVertex());
    kmf.atDecayPoint();
    unsigned err2 = kmf.fit();
    ...
    if(err == 0 && err2 == 0){
      ...
      dynamic_cast<UserInfo&>(plist[i].userInfo()).vx(kf.vertex().x(),0);
      dynamic_cast<UserInfo&>(plist[i].userInfo()).vy(kf.vertex().y(),0);
      dynamic_cast<UserInfo&>(plist[i].userInfo()).vz(kf.vertex().z(),0);
      dynamic_cast<UserInfo&>(plist[i].userInfo()).evx(kf.errVertex()[0][0],0);
      dynamic_cast<UserInfo&>(plist[i].userInfo()).evy(kf.errVertex()[1][1],0);
      dynamic_cast<UserInfo&>(plist[i].userInfo()).evz(kf.errVertex()[2][2],0);

      dynamic_cast<UserInfo&>(plist[i].userInfo()).CL(kf.cl() ,0); // D0 vertex
      dynamic_cast<UserInfo&>(plist[i].userInfo()).CL(kvf.cl(),1); // refit

      dynamic_cast<UserInfo&>(plist[i].userInfo()).dM(dm0 ,0);
      dynamic_cast<UserInfo&>(plist[i].userInfo()).dM(dm1 ,1);
    }else{
      HepPoint3D vtx(999.,999.,999.);
      HepSymMatrix errVtx(3,0);
      plist[i].momentum().decayVertex(vtx,errVtx);
...
void
dsd0pi_module::writeHisto(vector<Particle> &plist,
			  BelleTuple *hist)
{
  for(int i=0;i<plist.size();++i){
    // lund(charge) info.
    ...
    m_hist->column("dvx",dynamic_cast<UserInfo&>(plist[i].userInfo()).vx(0));
    m_hist->column("dvy",dynamic_cast<UserInfo&>(plist[i].userInfo()).vy(0));
    m_hist->column("dvz",dynamic_cast<UserInfo&>(plist[i].userInfo()).vz(0));
    m_hist->column("edvx2",dynamic_cast<UserInfo&>(plist[i].userInfo()).evx(0));
    m_hist->column("edvy2",dynamic_cast<UserInfo&>(plist[i].userInfo()).evy(0));
    m_hist->column("edvz2",dynamic_cast<UserInfo&>(plist[i].userInfo()).evz(0));

    m_hist->column("dcl",dynamic_cast<UserInfo&>(plist[i].userInfo()).CL(0));
    m_hist->column("refcl",dynamic_cast<UserInfo&>(plist[i].userInfo()).CL(1));

    m_hist->column("dm0",dynamic_cast<UserInfo&>(plist[i].userInfo()).dM(0));
    m_hist->column("dm1",dynamic_cast<UserInfo&>(plist[i].userInfo()).dM(1));
