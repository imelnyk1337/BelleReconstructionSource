...
class d0kpipipi_mv_module : public Module
{
public:
  d0kpipipi_mv_module(void);
  ~d0kpipipi_mv_module(void){};
  void init(int*);
  void term(void){};
...
extern "C" Module_descr *mdcl_d0kpipipi_mv()
{
  d0kpipipi_mv_module *module = new d0kpipipi_mv_module;
  Module_descr *dscr = new Module_descr ( "d0kpipipi_mv", module );
  return dscr;
}

d0kpipipi_mv_module::d0kpipipi_mv_module(void)
  : m_ptypeD0("D0"),
    m_mc(1)
{
}
...
void 
d0kpipipi_mv_module::hist_def(void)
{
...
void
d0kpipipi_mv_module::begin_run(BelleEvent *dummy,
			       int *dummyStatus)
{
  //eid::init_data();
  return;
...
void
d0kpipipi_mv_module::init(int *dummyStatus)
{
  //dummy
  Ptype dummy("E-");
}
...
void 
d0kpipipi_mv_module::endEvent(void)
{
  eraseVector(m_kaonP);
  eraseVector(m_kaonM);
  eraseVector(m_pionP);
...
void
d0kpipipi_mv_module::event(BelleEvent *evptr, int *status)
{
  *status = -1;
  ...
  if(m_mc){
    ...
    if(evtMgr.count() != 0){
      if(evtMgr[0].ExpMC() != 2){
        m_mc = 0; // not MC
        dout(Debugout::INFO,"d0kpipipi_mv") << "(module) This analysis is in REAL DATA not MC DATA." << std::endl;
      }
    }
  }
  ...
  combination(D0,  m_ptypeD0,  m_kaonM, m_pionM, m_pionP, m_pionP);
  setUserInfo(D0);
...
void
d0kpipipi_mv_module::massD0Event(vector<Particle> &plist)
{
  for(int i=0;i<plist.size();++i){
    ...
    addTrack2fit(kf, plist[i].child(3));
...
void
d0kpipipi_mv_module::writeHisto(vector<Particle> &plist,
				BelleTuple *hist)
{
  for(int i=0;i<plist.size();++i){
    // lund(charge) info.
