...
class d0kpi_m_module : public Module
{
public:
  d0kpi_m_module(void);
  ~d0kpi_m_module(void){};
...
extern "C" Module_descr *mdcl_d0kpi_m()
{
  d0kpi_m_module *module = new d0kpi_m_module;
  Module_descr *dscr = new Module_descr ( "d0kpi_m", module );
  return dscr;
}

d0kpi_m_module::d0kpi_m_module(void)
  : m_ptypeD0("D0"),
    m_mc(1)
{
}
...
void
d0kpi_m_module::begin_run(BelleEvent *dummy,
			  int *dummyStatus)
{
  //eid::init_data();
  return;
...
void
d0kpi_m_module::init(int *dummyStatus)
{
  //dummy
  Ptype dummy("E-");
}
...
void
d0kpi_m_module::event(BelleEvent *evptr, int *status)
{
  *status = -1;
  ...
  if(m_mc){
    ...
    if(evtMgr.count() != 0){
      if(evtMgr[0].ExpMC() != 2){
        m_mc = 0; // not MC
        dout(Debugout::INFO,"d0kpi_m") << "(module) This analysis is in REAL DATA not MC DATA." << std::endl;
      }
    }
  }
...
void
d0kpi_m_module::massD0Event(vector<Particle> &plist)
{
  for(int i=0;i<plist.size();++i){
    ...
    unsigned err = kf.fit();
    if(err == 0){
      /* dout(Debugout::INFO,"d0kpi_m") << "Error" << std::endl;
      dout(Debugout::INFO,"d0kpi_m") << kf.momentum(0) << std::endl;
      dout(Debugout::INFO,"d0kpi_m") << kf.momentum(1) << std::endl;
      dout(Debugout::INFO,"d0kpi_m") << kf.position(0) << std::endl;
      dout(Debugout::INFO,"d0kpi_m") << kf.position(1) << std::endl;
      dout(Debugout::INFO,"d0kpi_m") << kf.error(0) << std::endl;
      dout(Debugout::INFO,"d0kpi_m") << kf.error(1) << std::endl;
      dout(Debugout::INFO,"d0kpi_m") << kf.vertex() << std::endl;
      dout(Debugout::INFO,"d0kpi_m") << kf.correlation(0,1) << std::endl; */
      kmakemother kmm;
      ...
      /* dout(Debugout::INFO,"d0kpi_m") << kmm.momentum() << std::endl;
      dout(Debugout::INFO,"d0kpi_m") << kmm.position() << std::endl;
      dout(Debugout::INFO,"d0kpi_m") << kmm.error() << std::endl; */
...
void
d0kpi_m_module::writeHisto(vector<Particle> &plist,
			   BelleTuple *hist)
{
  for(int i=0;i<plist.size();++i){
    // lund(charge) info.
